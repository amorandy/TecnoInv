...
function get_operador_info ($email){
    
require "config.php";

$sql = "SELECT id,name FROM operadores WHERE email = '$email'";
...
function get_index_week_data ($id){
    ...
    $sql = "SELECT id FROM transfers WHERE DATE(date) >= '".date("Y-m-d", strtotime ("-7days"))."' AND DATE(date) <= '".date("Y-m-d")."' AND operador_id = '$id'";
    $rs1 = $db_connect -> query($sql);
    //-----------------------------------------
    $sql = "SELECT SUM(monto_usd) AS total FROM transfers WHERE DATE(date) >= '".date("Y-m-d", strtotime ("-7days"))."' AND DATE(date) <= '".date("Y-m-d")."' AND operador_id='$id'";
    $rs = $db_connect -> query($sql);
    $arr=mysqli_fetch_array($rs);
    //-----------------------------------------
    $sql = "SELECT SUM(monto_usd) AS total FROM transfers WHERE DATE(date) >= '".date("Y-m-d", strtotime ("-7days"))."' AND DATE(date) <= '".date("Y-m-d")."' AND operador_id='$id'";
...
function get_transaction_info ($id){
	...
	define("operador_imagen", $arr["operador_imagen"]);
...
function sendMessageAtt ($id,$mid,$path){
