    ...
    if($_POST['name']==NULL|$_POST['email']==NULL|$_POST['password']==NULL|$_POST['status']==NULL|$_POST['billetera']==NULL) { 
        ...
        $error = 'Todos los campos son requeridos';
            ...
            $billetera_id = $_POST['billetera'];
             ...
             $sql = ("INSERT INTO operadores (name,email,password,status,billetera_id) VALUES ('$name','$email','".password_hash($password, PASSWORD_DEFAULT)."','$status','$billetera_id')");
             $csql = $db_connect -> query($sql);
         
/*
        foreach($moneda_id as $index => $value){

           $sql12 = ("INSERT INTO wallet (operador_id,divisa_id,monto) VALUES ('$o_id','".$moneda_id[$index]."','".$monto[$index]."')");
...
<?php  
include "config/config.php";                                      
$tabla2 = ("SELECT id,name,email,status FROM operadores ORDER BY id DESC"); 
$rs2 = $db_connect -> query($tabla2);

while ($registro = mysqli_fetch_array($rs2)) { 
    
    $sql11 = "SELECT name FROM billetera WHERE operador_id='".$registro['id']."'";
...
?>
                                            ...
                                            <tr>
                                                ...
                                                <td><a href="edit_operador?id=<?php echo $registro['id']; ?>&edit=1"><i class="fa fa-pencil" aria-hidden="true"></i>
</a></td>
                                            </tr>
<?php 
} 
